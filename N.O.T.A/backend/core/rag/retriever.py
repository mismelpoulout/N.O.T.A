# -*- coding: utf-8 -*-
from __future__ import annotations
import math, re, sqlite3, json, time
from dataclasses import dataclass
from typing import List, Tuple, Dict, Optional
from collections import defaultdict, Counter

# --------- util ----------
_token = re.compile(r"[a-z0-9áéíóúüñ]+")

def tokenize(text: str) -> List[str]:
    return _token.findall((text or "").lower())

def l2(a: List[float]) -> float:
    return math.sqrt(sum(x*x for x in a)) or 1.0

# --------- Dense embeddings (degradado) ----------
class DenseBackend:
    """
    Intenta fastembed -> sentence-transformers -> hashing-bow mean-vector
    Todas devuelven vectores list[float]
    """
    def __init__(self, model: str | None = None):
        self.kind = "hashing"
        self.dim = 256

        try:
            from fastembed import TextEmbedding
            self.model = TextEmbedding(model or "BAAI/bge-small-en-v1.5")
            self.kind = "fastembed"
            self.dim = len(self.model.embed(["ok"])[0])
        except Exception:
            try:
                from sentence_transformers import SentenceTransformer
                self.model = SentenceTransformer(model or "sentence-transformers/all-MiniLM-L6-v2")
                self.kind = "sbert"
                self.dim = self.model.get_sentence_embedding_dimension()
            except Exception:
                self.model = None  # hashing fallback

    def embed(self, texts: List[str]) -> List[List[float]]:
        if self.kind == "fastembed":
            return [list(v) for v in self.model.embed(texts)]
        if self.kind == "sbert":
            import numpy as np
            arr = self.model.encode(texts, normalize_embeddings=True)
            return [list(v) for v in arr]
        # hashing fallback (bag of words into fixed dim)
        vecs = []
        for t in texts:
            v = [0.0]*self.dim
            for tok in tokenize(t):
                h = hash(tok) % self.dim
                v[h] += 1.0
            n = l2(v)
            vecs.append([x/n for x in v])
        return vecs

# --------- BM25 backend (degradado) ----------
class BM25Backend:
    def __init__(self, con: sqlite3.Connection):
        self.con = con
        try:
            from rank_bm25 import BM25Okapi  # optional
            self._has_rankbm25 = True
            self.BM25Okapi = BM25Okapi
        except Exception:
            self._has_rankbm25 = False

        self._corpus: Dict[str, List[str]] = {}   # chunk_id -> tokens
        cur = self.con.execute("SELECT chunk_id, tokens FROM bm25_index")
        for cid, toks in cur.fetchall():
            self._corpus[cid] = (toks or "").split()

        if self._has_rankbm25:
            self._bm = self.BM25Okapi(list(self._corpus.values()))
            self._ids = list(self._corpus.keys())
        else:
            # precompute idf for simple bm25
            N = max(1, len(self._corpus))
            df: Counter = Counter()
            for toks in self._corpus.values():
                for t in set(toks):
                    df[t] += 1
            self._idf = {t: math.log((N - df[t] + 0.5)/(df[t] + 0.5) + 1) for t in df}

    def search(self, query: str, top_k: int = 50) -> List[Tuple[str, float]]:
        if not self._corpus:
            return []
        q_toks = tokenize(query)
        if self._has_rankbm25:
            scores = self._bm.get_scores(q_toks)
            pairs = list(zip(self._ids, scores))
            pairs.sort(key=lambda x: x[1], reverse=True)
            return pairs[:top_k]
        # simple bm25-ish
        k1, b = 1.5, 0.75
        avgdl = sum(len(v) for v in self._corpus.values())/len(self._corpus)
        out = []
        for cid, toks in self._corpus.items():
            dl = len(toks) or 1
            score = 0.0
            tf = Counter(toks)
            for t in q_toks:
                if t not in tf or t not in self._idf: 
                    continue
                num = tf[t]*(k1+1)
                den = tf[t] + k1*(1 - b + b*dl/avgdl)
                score += self._idf[t] * (num/den)
            out.append((cid, score))
        out.sort(key=lambda x: x[1], reverse=True)
        return out[:top_k]

# --------- Hybrid retriever ----------
@dataclass
class Chunk:
    id: str
    url: str
    title: str
    section: str
    text: str
    published_at: float | None
    evidence_score: float

class HybridRetriever:
    """
    Fusiona búsqueda densa y lexical y aporta metadatos de doc_chunks.
    """
    def __init__(self, db_path: str, dense_model: str | None = None):
        self.db_path = db_path
        self.con = sqlite3.connect(db_path)
        self.con.row_factory = sqlite3.Row
        self.dense = DenseBackend(dense_model)
        self.bm25 = BM25Backend(self.con)

        # carga embeddings si existen
        self._embs: Dict[str, List[float]] = {}
        try:
            cur = self.con.execute("SELECT chunk_id, embedding_json FROM vec_index")
            for cid, ej in cur.fetchall():
                self._embs[cid] = json.loads(ej)
        except Exception:
            # tabla opcional
            pass

    # --------- util: coseno ----------
    @staticmethod
    def _cos(a: List[float], b: List[float]) -> float:
        num = sum(x*y for x,y in zip(a,b))
        return num/(l2(a)*l2(b))

    def _load_chunk(self, cid: str) -> Optional[Chunk]:
        c = self.con.execute(
            "SELECT id,url,title,section,chunk,published_at,evidence_score FROM doc_chunks WHERE id=?",
            (cid,)
        ).fetchone()
        if not c: return None
        return Chunk(
            id=c["id"], url=c["url"] or "", title=c["title"] or "", section=c["section"] or "otros",
            text=c["chunk"] or "", published_at=c["published_at"], evidence_score=c["evidence_score"] or 0.0
        )

    def search(self, query: str, top_k_dense=50, top_k_lex=50) -> List[Dict]:
        # denso
        qv = self.dense.embed([query])[0]
        dense_scores: Dict[str, float] = {}
        for cid, emb in self._embs.items():
            dense_scores[cid] = self._cos(qv, emb)

        dense_top = sorted(dense_scores.items(), key=lambda x: x[1], reverse=True)[:top_k_dense]
        # léxico
        lex_top = self.bm25.search(query, top_k=top_k_lex)

        # fusión
        pool: Dict[str, Dict] = defaultdict(lambda: {"cos":0.0, "bm25":0.0})
        for cid, s in dense_top: pool[cid]["cos"] = s
        for cid, s in lex_top:   pool[cid]["bm25"] = s

        results = []
        for cid, feats in pool.items():
            ch = self._load_chunk(cid)
            if not ch: 
                continue
            feats.update({
                "chunk_id": ch.id,
                "url": ch.url,
                "title": ch.title,
                "section": ch.section,
                "chunk": ch.text,
                "published_at": ch.published_at or 0.0,
                "evidence_score": ch.evidence_score or 0.0
            })
            results.append(feats)
        return results

    # --------- indexado (para ingest) ----------
    def upsert_embedding(self, chunk_id: str, text: str):
        v = self.dense.embed([text])[0]
        self.con.execute("""
            CREATE TABLE IF NOT EXISTS vec_index(
              chunk_id TEXT PRIMARY KEY,
              embedding_json TEXT
            )
        """)
        self.con.execute("REPLACE INTO vec_index(chunk_id, embedding_json) VALUES(?,?)",
                         (chunk_id, json.dumps(v)))
        self.con.commit()

    def upsert_bm25_tokens(self, chunk_id: str, text: str):
        toks = " ".join(tokenize(text))
        self.con.execute("REPLACE INTO bm25_index(chunk_id, tokens) VALUES(?,?)",
                         (chunk_id, toks))
        self.con.commit()